...
void VMTTimelineRuler::_draw() {
    ...
    float start = rect.position.x - fmod(rect.position.x, scale * 50);
    float end = rect.position.x + rect.size.x;

    for (float i = start - rect.position.x; i < end - rect.position.x; i += scale * 200) {
        draw_line(Vector2(i, 0), Vector2(i, 20), Color(0.5, 0.5, 0.5, 1));
        auto time = VisualMovieTab::get_singleton()->to_time_seperated(i + rect.position.x);
